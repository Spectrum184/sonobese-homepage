...
let gsap,
  ...
  _getProxyProp = (element, property) =>
  ...
  _getBoundsFunc = (element) =>
    ...
    (_isViewport(element)
      ...
      : () => _getBounds(element)),
  _getSizeFunc = (scroller, isViewport, { d, d2, a }) =>
    (a = _getProxyProp(scroller, "getBoundingClientRect"))
      ? () => a()[d]
      : () => (isViewport ? _win["inner" + d2] : scroller["client" + d2]) || 0,
  _getOffsetsFunc = (element, isViewport) =>
    !isViewport || ~_proxies.indexOf(element)
      ? _getBoundsFunc(element)
      : () => _winOffsets,
  _maxScroll = (element, { s, d2, d, a }) =>
    (s = "scroll" + d2) && (a = _getProxyProp(element, s))
      ? a() - _getBoundsFunc(element)()[d]
      : _isViewport(element)
      ? Math.max(_docEl[s], _body[s]) -
  ...
  _makePositionable = (element) =>
    (element.style.position =
      _getComputedStyle(element).position === "absolute"
        ? "absolute"
        : "relative"), // if the element already has position: absolute, leave that, otherwise make it position: relative
  ...
  _getClosestLabel = (animation) => (value) =>
    gsap.utils.snap(_getLabelRatioArray(animation), value),
  ...
  _multiListener = (func, element, types, callback) =>
    types.split(",").forEach((type) => func(element, type, callback)),
  _addListener = (element, type, func) =>
    element.addEventListener(type, func, { passive: true }),
  ...
  _createMarker = (
    ...
    container,
    direction,
    { startColor, endColor, fontSize, indent, fontWeight },
    offset,
    matchWidthEl
  ) => {
    ...
    css +=
      "position:" + (isScroller && useFixedPosition ? "fixed;" : "absolute;");
    (isScroller || !useFixedPosition) &&
      (css +=
  ...
  _softRefresh = () =>
    ...
    _refreshAll(true),
  _dispatch = (type) =>
    (_listeners[type] && _listeners[type].map((f) => f())) || _emptyArray,
  ...
  _refreshAll = (force, skipRevert) => {
    ...
    refreshInits.forEach(
      (result) => result && result.render && result.render(-1)
  ...
  _swapPinIn = (pin, spacer, cs, spacerState) => {
    if (pin.parentNode !== spacer) {
      ...
      spacerStyle[_padding] = pinStyle[_margin] = pinStyle[_top] = pinStyle[
        _left
      ] = "0";
    ...
    if (marker) {
      ...
      _positionMarker(
        ...
        (isStart && position > 20) ||
          (!isStart &&
            (useFixedPosition
              ? Math.max(_body[scrollerMax], _docEl[scrollerMax])
              : marker.parentNode[scrollerMax]) <=
              position + 1)
      );
      if (useFixedPosition) {
...
modules.export = class ScrollTrigger {
  ...
  init(vars, animation) {
    ...
    let direction = vars.horizontal ? _horizontal : _vertical,
      ...
      useFixedPosition =
        "pinType" in vars
          ? vars.pinType === "fixed"
          : isViewport || _getProxyProp(scroller, "pinType") === "fixed",
      callbacks = [
        vars.onEnter,
        vars.onLeave,
        vars.onEnterBack,
    ...
    if (snap) {
      ...
      snapDelayedCall = gsap
        .delayedCall(snap.delay || scrubSmooth / 2 || 0.1, () => {
          if (Math.abs(self.getVelocity()) < 10 && !_pointerIsDown) {
            let totalProgress =
              ...
              naturalEnd =
                totalProgress + (snap.inertia === false ? 0 : change1),
            ...
            if (scroll <= end && scroll >= start && endScroll !== scroll) {
              if (
                tween &&
                !tween._initted &&
                tween.data <= Math.abs(endScroll - scroll)
              ) {
    ...
    self.refresh = (soft, force) => {
      ...
      if (pin) {
        ...
        !max &&
          ...
          ((isViewport ? _body : scroller).style["overflow-" + direction.a] =
            "scroll"); // makes sure the scroller has a scrollbar, otherwise if something has width: 100%, for example, it would be too big (exclude the scrollbar). See https://greensock.com/forums/topic/25182-scrolltrigger-width-of-page-increase-where-markers-are-set-to-false/
        _swapPinIn(pin, spacer, cs);
        pinState = _getState(pin);
      ...
      for (i = 0; i < triggerIndex; i++) {
        ...
        curTrigger &&
          (curTrigger === trigger || curTrigger === pin) &&
          _triggers[i].revert(false);
      }
      self.start = start;
      self.end = end;
    ...
    self.getVelocity = () =>
      ((self.scroll() - scroll2) / (_getTime() - _time2)) * 1000 || 0;
    ...
    self.update = (reset, recordVelocity) => {
      ...
      if (clipped !== prevProgress && self.enabled) {
            ...
            if (pinReparent) {
              if (!reset && (isActive || action)) {
                ...
                _reparent(
                  pin,
                  _body,
                  bounds.top + (direction === _vertical ? offset : 0) + _px,
                  bounds.left + (direction === _vertical ? 0 : offset) + _px
                );
              } else {
        ...
        toggleClass &&
          ...
          _toArray(toggleClass.targets).forEach((el) =>
            el.classList[isActive || once ? "add" : "remove"](
    ...
    self.enable = () => {
      if (!self.enabled) {
        ...
        !animation || !animation.add
          ? self.refresh()
          : gsap.delayedCall(0.01, () => start || end || self.refresh()) &&
            (change = 0.01) &&
            (start = end = 0); // if the animation is a timeline, it may not have been populated yet, so it wouldn't render at the proper place on the first refresh(), thus we should schedule one for the next tick.
      }
    };
    ...
    if (!_coreInitted) {
      ...
      if (gsap) {
        ...
        if (_body) {
          ...
          border
            ? (bodyStyle.borderTop = border)
            : bodyStyle.removeProperty("border-top");
...
ScrollTrigger.maxScroll = (element, horizontal) =>
  _maxScroll(element, horizontal ? _horizontal : _vertical);
ScrollTrigger.getScrollFunc = (element, horizontal) =>
  _getScrollFunc(_toArray(element)[0], horizontal ? _horizontal : _vertical);
